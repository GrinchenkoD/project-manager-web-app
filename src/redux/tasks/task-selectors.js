...
export const chartDaysSelector = list[0].hoursWastedPerDay.map((task) => task.currentDay);

export const hoursPlannedSelector = list.map((task) =>
Number(task.hoursPlanned))
.reduce((acc, taskValue) => {
  return (acc += taskValue);
}, 0);

export const sumHoursWastedSelector = list.map((task) => Number(task.hoursWasted))
    .reduce((acc, taskValue) => {
      return (acc += taskValue);
    }, 0)
